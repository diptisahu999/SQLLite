...
def turn_on():
    ...
    def relay_opr(param):
        ...
        print(deviceid_hex,"devices hexx")
        ...
        if operation != None:
            ...
            print("devices_operationssss ",opr)
            print("gettttT   " , rc.OPERATION + device_id + channel_id + opr + delay)
            get = crc16_ccitt(rc.OPERATION + device_id + channel_id + opr + delay, crc=0)
            print(get,"getttttttttttttt")
            crc_int = hex(get)[2:]
            if len(crc_int) == 3:
                crc_int = '0' + crc_int
            if len(crc_int) == 2:
            ...
            a = sm.replace('\\x', '')
            a.encode('utf8')
            print("CRC encoder values ",a.encode('utf8'))
            x = binascii.a2b_hex(a.encode('utf8'))
            print("headers" , rc.HEADER)
            print("Operations" , rc.OPERATION)
            print("Devices_id " , device_id)
            print("Channels id" , channel_id)
            print("devices_operations",opr)
            print("Additional--",rc.TRAIL+ x)
